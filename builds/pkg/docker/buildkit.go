...
func ClientOpts(c client.HijackDialer) []bkclient.ClientOpt {
	return []bkclient.ClientOpt{
		bkclient.WithSessionDialer(SessionDialer(c)),
...
// SessionDialer returns a BuildKit session dialer backed by the Docker API.
func SessionDialer(c client.HijackDialer) session.Dialer {
